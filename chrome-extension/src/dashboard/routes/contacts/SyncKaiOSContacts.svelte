<script lang="ts">
    ...
    import KaiOSSyncGuide from '../../widgets/guide/KaiOSSync.svelte';
                ...
                if (c.key && c.key.length > 0) {
                    ...
                    for (let i in c.key) {
                        if (davContactListIndex[c.key[i]] != null) {
                            temp1.push({ kaios: c.id, carddav: c.key[i], status: true });
                            push = false;
                            break;
                        }
                    }
                    if (push)
                        temp2.push({ kaios: c.id, carddav: null, status: false });
                } else
                    temp2.push({ kaios: c.id, carddav: null, status: false });
            });
...
<div>
    <div class="d-flex flex-row justify-content-between align-items-center">
        ...
        <div class="d-flex flex-row">
            <button type="button" class="btn btn-primary btn-sm me-1" on:click={() => { openModal(KaiOSSyncGuide) }}>Guide</button>
